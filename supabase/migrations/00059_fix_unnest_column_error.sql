-- Fix critical SQL error: column "unnest" does not exist
...
BEGIN
  ...
  -- FIXED: Remove empty strings and very short words (correct syntax)
  v_keywords := ARRAY(
  ...
  WHERE 
    ...
    -- Strategy 3: Term matches any keyword
    OR EXISTS (
      SELECT 1 FROM unnest(v_keywords) AS kw
      WHERE term ILIKE '%' || kw || '%' AND length(kw) >= 2
    )
    -- Strategy 4: Normalized term matches any keyword
    OR EXISTS (
      SELECT 1 FROM unnest(v_keywords) AS kw
  ...
  -- PCB specific checks
  IF v_normalized_text LIKE '%pcb%' OR v_normalized_text LIKE '%placa%' OR v_normalized_text LIKE '%circuito%' OR v_normalized_text LIKE '%queimado%' THEN
    v_checklist := v_checklist || jsonb_build_array(
