-- Fix the get_ai_diagnostic function - remove JSONB concatenation issues
...
BEGIN
  ...
  -- Initialize technical observations array
  v_technical_observations := ARRAY[]::TEXT[];
  
  -- Generate probable causes and other data based on category
  IF v_category = 'Bateria' THEN
    ...
    v_technical_observations := ARRAY[
  ...
  ELSE
    ...
    v_technical_observations := ARRAY[
      '📋 Documentar todas as observações e testes realizados',
      '🔍 Solicitar mais informações ao cliente se necessário'
    ];
  END IF;
  ...
  v_result := jsonb_build_object(
    ...
    'diagnosis', jsonb_build_object(
      ...
      'technical_observations', array_to_json(v_technical_observations),
...
EXCEPTION WHEN OTHERS THEN
  ...
  RETURN jsonb_build_object(
    ...
    'diagnosis', jsonb_build_object(
      ...
      'technical_observations', '["Erro ao processar diagnóstico. Tente novamente."]'::jsonb,
      'complexity', 'medium'
    )
  );
END;
