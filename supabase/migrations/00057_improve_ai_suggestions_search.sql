-- Improve AI suggestions RPC function to better search knowledge base
...
BEGIN
  ...
  -- Extract keywords (simple split by space and remove punctuation)
  v_keywords := string_to_array(regexp_replace(v_normalized_text, '[^\w\s]', '', 'g'), ' ');
  
  -- Find matching terms in database - improved search
  -- Search by exact match, partial match, or any keyword match
  ...
  WHERE 
    ...
    -- Or partial match in the text
    OR v_normalized_text LIKE '%' || normalized_term || '%'
    -- Or term appears in the text
    OR term ILIKE '%' || ANY(v_keywords) || '%'
    -- Or any keyword matches the term
    OR EXISTS (
