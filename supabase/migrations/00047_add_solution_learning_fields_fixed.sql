-- Add missing fields for Solution Learning feature
...
CREATE INDEX IF NOT EXISTS idx_ai_knowledge_events_event_type ON ai_knowledge_events(event_type);
CREATE INDEX IF NOT EXISTS idx_ai_knowledge_events_causa_raiz ON ai_knowledge_events(causa_raiz) WHERE causa_raiz IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_ai_knowledge_events_tags_solucao ON ai_knowledge_events USING GIN(tags_solucao) WHERE tags_solucao IS NOT NULL;

-- Create RPC function to register solutions
...
BEGIN
  ...
  v_normalized_terms := ARRAY(
    SELECT unnest(v_keywords)
    WHERE unnest ~ '(bateria|tela|display|placa|agua|molh|oxid|queima|curto|modchip|chip|ic|capacitor|resistor|soldagem|troca|reparo|limpeza)'
  );
  ...
  SET 
    status = CASE 
      WHEN status NOT IN ('completed', 'delivered') THEN 'completed'
      ELSE status
    END,
    completed_at = CASE
...
GRANT EXECUTE ON FUNCTION register_solution TO authenticated;